...
% a fastScan recording it's what was turning a <1 min ImageJ open into a
% 9 min MATLAB one. This version opens the file once with the Tiff class
% and steps through directories sequentially with nextDirectory(), which
% is O(N) - the way libtiff is meant to be driven for a multi-page file.
% Output class/values are unchanged (still double) so nothing downstream
% (e.g. thresholdVesselIm's nanmean/nanstd, which can error on integer
% input) is affected.
