...
% =============================================================================
logoPath = fullfile(mapsDir, 'MAPSlogo.png');
if isfile(logoPath)
    uiimage(fig, 'ImageSource', logoPath, 'Position', [8 788 68 66]);
end

uilabel(fig, ...
    'Position',   [85 818 700 24], ...
    'Text',       'Microvascular Analysis & Phenotyping Suite  (MAPS)', ...
    'FontSize',   17, 'FontWeight', 'bold', ...
    'FontColor',  [0.15 0.15 0.15]);
uilabel(fig, ...
    'Position',   [85 797 600 20], ...
    'Text',       'Vessel diameter analysis from two-photon imaging', ...
    'FontSize',   11, 'FontColor', [0.40 0.40 0.40]);

% thin separator line below header
uipanel(fig, 'Position', [0 782 1430 2], 'BackgroundColor', [0.7 0.7 0.75], ...
    'BorderType', 'none');
...
uilabel(fig, 'Position', [145 754 68 20], 'Text', 'Analysis:', ...
    'FontColor', [0.25 0.25 0.25]);
ddAnalysis = uidropdown(fig, ...
    'Position', [213 749 130 28], ...
    'Items',    {'xyDiam', 'zstack', 'linescan'}, ...
    'Value',    'xyDiam', 'FontSize', 12);

% ---- Vessel display axes ----------------------------------------------------
uilabel(fig, 'Position', [LX 729 250 18], ...
    'Text', 'Vessel display  (frame ~50)', ...
    'FontSize', 10, 'FontColor', [0.35 0.35 0.35]);

dispAx = uiaxes(fig, 'Position', [LX 320 LW 406]);
...
uilabel(fig, 'Position', [LX 300 200 18], ...
    'Text', 'Processing options', ...
    'FontSize', 12, 'FontWeight', 'bold', 'FontColor', [0.15 0.15 0.15]);

btnSkel = uibutton(fig, ...
    'Position',        [LX 265 172 28], ...
    'Text',            'Generate skeleton', ...
    'ButtonPushedFcn', @(~,~) cb_generateSkeleton());

lblSkelTick = uilabel(fig, 'Position', [188 263 30 30], ...
    'Text', '', 'FontSize', 20, 'FontColor', [0.10 0.70 0.20]);

btnBranch = uibutton(fig, ...
    'Position',        [LX 228 210 28], ...
    'Text',            'Draw around vessel branch', ...
    'ButtonPushedFcn', @(~,~) cb_drawBranch());

lblBranchTick = uilabel(fig, 'Position', [226 226 30 30], ...
    'Text', '', 'FontSize', 20, 'FontColor', [0.10 0.70 0.20]);

% ---- Parameters panel -------------------------------------------------------
pnl = uipanel(fig, ...
    'Position',        [LX 52 LW 170], ...
    'Title',           'Parameters', ...
    'FontWeight',      'bold', ...
    'BackgroundColor', [1 1 1]);

uilabel(pnl, 'Position', [10 115 115 22], 'Text', 'No. of branches:');
efNBranch = uieditfield(pnl, 'numeric', ...
    'Position', [130 115 55 22], 'Value', 1, 'Limits', [1 5]);

uilabel(pnl, 'Position', [10 80 115 22], 'Text', 'Pixel size (microns):');
efPxsz = uieditfield(pnl, 'text', ...
    'Position', [130 80 100 22], 'Value', '', 'Placeholder', 'blank = pixels');

uilabel(pnl, 'Position', [10 46 115 22], 'Text', 'Frame rate (Hz):');
efFPS = uieditfield(pnl, 'text', ...
    'Position', [130 46 100 22], 'Value', '', 'Placeholder', 'blank = frames');

uilabel(pnl, 'Position', [10 12 LW-20 28], ...
    'Text',      'Leave blank to output diameter in pixels / time in frames.', ...
    'FontSize',  9, 'FontColor', [0.50 0.50 0.50], ...
    'WordWrap',  'on');

% ---- GO button --------------------------------------------------------------
btnGo = uibutton(fig, ...
    'Position',        [LX 14 LW 33], ...
    'Text',            'GO', ...
...
% ---- Processing updates -----------------------------------------------------
uilabel(fig, 'Position', [RX 762 200 18], ...
    'Text', 'Processing updates', ...
    'FontSize', 10, 'FontWeight', 'bold', 'FontColor', [0.25 0.25 0.25]);
txaUpdates = uitextarea(fig, ...
    'Position', [RX 730 RW 30], ...
    'Editable', false, ...
    'Value',    'Load data to begin.', ...
    'FontSize', 11);

% ---- Diameter heatmap axes --------------------------------------------------
uilabel(fig, 'Position', [RX 708 350 18], ...
    'Text', 'Diameter map', ...
    'FontSize', 10, 'FontWeight', 'bold', 'FontColor', [0.25 0.25 0.25]);
heatAx = uiaxes(fig, 'Position', [RX 415 RW 290]);
...
uilabel(fig, 'Position', [RX 393 350 18], ...
    'Text', 'Average diameter  (mean across skeleton pts)', ...
    'FontSize', 10, 'FontWeight', 'bold', 'FontColor', [0.25 0.25 0.25]);
traceAx = uiaxes(fig, 'Position', [RX 55 RW 335]);
...
btnExportFig = uibutton(fig, ...
    'Position',        [RX+btnW2+10 14 btnW2 33], ...
    'Text',            'Export figure', ...
    'FontSize',        13, 'FontWeight', 'bold', ...
...
uilabel(fig, ...
    'Position',            [RX+RW-140 1 140 12], ...
    'Text',                'Shaw (2026)', ...
    'FontSize',            9, 'FontColor', [0.40 0.40 0.40], ...
