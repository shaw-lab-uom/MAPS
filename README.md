...
- **ZScore** — expresses the signal relative to the *variability* of a quiescent baseline rather than its resting *level*, after Longden et al. (2021, see [References](#references)), who found a z-score more sensitive to very small, sparse events and more robust to the wide range of basal indicator brightness across capillaries.
  - **Spatial unit — one skeleton point.** The z-score is computed **per skeleton point** (i.e. per perpendicular line across the branch), never per pixel and never per vessel edge. As everywhere in this pipeline, the In→Out calcium band on the *two* FWHM edges of that line is first pooled into a **single background-corrected value per skeleton point per frame** (`F_bgcorr`, see the "both vessel edges are pooled" note above) — that pooled value is what gets z-scored.
