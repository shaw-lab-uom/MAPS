...
- **ZScore** — expresses the signal relative to the *variability* of a quiescent baseline rather than its resting *level*, after Longden et al. (2021, see [References](#references)), who found a z-score more sensitive to very small, sparse events and more robust to the wide range of basal indicator brightness across capillaries.
  ...
  - **How "quiescent" is defined.** Longden identifies the quiet epoch from independent behavioural data (no locomotion / activity). MAPS has no such channel, so "quiet" is defined data-adaptively as the dimmest **Quiet %** of that skeleton point's finite frames. If a point is active for more than roughly (100 − Quiet %) % of the recording its baseline is contaminated — lower **Quiet %**, or exclude that point downstream. Guards: a point needs ≥ 10 finite frames; if fewer than 5 fall in the quiet fraction, all finite frames are used instead; if SD_q is not > 0 the point is left as `NaN`.
