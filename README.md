...
- **Export**: results as a `.mat` file or as one Excel file per branch (frame, time in seconds, average diameter, per-skeleton-point diameters, plus every stage of the perivascular calcium pipeline if used), and a multi-panel summary figure (vessel + skeleton + ROI + perpendicular lines, diameter heatmap, average trace — plus, if calcium was run, the calcium channel with the same perp lines, the actual sampled pixels, and the average dF/F0 trace) as PNG/PDF/FIG.

**Perivascular calcium (optional):** ticking **Perivascular Calcium** in the Preprocessing panel prompts for a second TIF (same field of view as the vessel channel) and, once **GO** is run, samples fluorescence around each FWHM-detected vessel edge, per skeleton point/branch/frame — the same edge-expansion approach as the underlying `FWHM_diam_perivascCa_adapted.m` script. Five numeric fields sit next to/below the checkbox, all editable at any time (not just once ticked) and all with sensible defaults, so you only need to touch the ones you want to change:
...
2. In **Processing options**: set the number of branches, then **Generate skeleton** and **Draw around vessel branch** for each one. Tick **Perivascular Calcium** and select a second TIF if you want it (adjust the In/Out/BG ring/r/Baseline fields if the defaults don't suit).
...
### z-score normalisation for perivascular calcium (option)

MAPS currently reports the perivascular signal as **ΔF/F₀** (background-ring subtracted, sliding-maximin baseline). Dividing by F₀ scales the signal by the resting *level*, so on dim vessels or where the baseline is uncertain (e.g. after background over-subtraction) the noise is inflated and small, sparse events can be lost.

Longden et al. (2021, *Sci. Adv.* 7:eabh0101) instead express endothelial capillary Ca²⁺ as a **z-score** relative to the *variability* of a quiescent baseline — reported to be more sensitive to very small events and more robust to the wide range of basal indicator brightness. Their spatial (statistically defined active-site masking) machinery is **not** needed here: MAPS already anchors sampling to the tracked vessel edges, so only the normalisation step changes.

Plan: a **"z-score (Longden 2021)"** checkbox in the Perivascular Calcium controls. When ticked, in place of ΔF/F₀ = (F_bgcorr − F₀)/F₀, output **z = (F_bgcorr − F_q) / SD_q**, where F_q and SD_q are the mean and SD of a quiescent baseline (first pass: the dimmest ~35 % of samples per skeleton point; later, Longden's `10·SD(minima)` → recompute in quiescent regions → SD_q). Both traces would be kept in the export; the choice affects only the live/plotted trace and its label. The z-score amplitude is in "SDs above rest", not a physiological %, so it is a detection/sensitivity aid rather than a replacement for ΔF/F₀. **Open question to settle with real data: does the z-score catch small perivascular transients that ΔF/F₀ misses?** No code or raw data was released with the Longden paper (custom tools *VolumetryG9*, G.W. Hennig; *SparkAn*, A. Bonev, are named but request-only).
