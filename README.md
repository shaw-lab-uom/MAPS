...
- **ZScore** — expresses the signal relative to the *variability* of a quiescent baseline rather than its resting *level*, after Longden et al. (2021, see [References](#references)), who found a z-score more sensitive to very small, sparse events and more robust to the wide range of basal indicator brightness across capillaries.
  - **Spatial unit — one skeleton point.** A *skeleton point* is one location on the vessel **centreline** (the *k*-th ordered point along the drawn skeleton); each one owns a single line drawn perpendicular to the vessel there. The z-score is computed **per skeleton point**, never per pixel and never per vessel edge. The value it works on is *not* the centre pixel and *not* the whole perpendicular line: it is the calcium in the **In→Out band straddling each of the two FWHM edges** of that perpendicular line (the two wall-hugging segments — the lumen and everything past the Out band are excluded), averaged together and background-ring–subtracted into a **single value per skeleton point per frame** (`F_bgcorr`, see the "both vessel edges are pooled" note above). That value is what gets z-scored.
  - **Each skeleton point is referenced to its own baseline.** For skeleton point *k*, MAPS takes *that point's own* `F_bgcorr` time series over the *whole recording*, identifies the frames where it sits in its dimmest **Quiet %** (default 35 %, matching Longden), and takes the mean and SD of only those frames: **z(k, t) = (F_bgcorr(k, t) − mean_q(k)) / SD_q(k)**. Every skeleton point therefore gets its own resting level *and* its own resting variability — the per-skeleton-point analogue of Longden's per-cell / per-pixel baseline. Skeleton points are combined only *after* z-scoring (the branch trace is the mean z across points; the per-point matrix is kept in the export).
  - **How "quiescent" is defined.** As in Longden et al., the quiescent baseline is taken **statistically from the fluorescence itself**, not from a separately recorded rest period — the dimmest **Quiet %** of that skeleton point's own finite frames (default 35 %) supplies `mean_q` and `SD_q`. (Both their preparation and typical MAPS test data are anaesthetised, so there is no locomotion signal to key on in either case.) The caveat: if a skeleton point is active for more than roughly (100 − Quiet %) % of the recording its "quiet" fraction is contaminated by real signal — lower **Quiet %**, or exclude that point downstream. Guards: a point needs ≥ 10 finite frames; if fewer than 5 fall in the quiet fraction, all finite frames are used instead; if SD_q is not > 0 the point is left as `NaN`.
